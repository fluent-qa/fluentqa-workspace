...
public class XlsxCellMapper<T> {
  ...
  public XlsxCellMapper(
    ...
    this.index =
        getColumn()
            ...
            .map(headers::get)
            .orElseThrow(
                () -> new IllegalArgumentException(format("Column %s not found", field.getName())));
  ...
  public XlsxCellMapper<T> parse(final Row row) {
    var cellValue =
        ofNullable(row.getCell(index, RETURN_BLANK_AS_NULL))
  ...
  public <R> void assignValue(final R instance) {
    ofNullable(convertedValue).ifPresent(value -> on(instance).set(getFieldName(), value));
  }
  ...
  private IConverter<T> findMatchingConverter() {
    return getColumn()
        ...
        .orElseGet(
            () ->
                StreamEx.of(defaultIConverters)
                    .findFirst(
