    ...
    public StreamEx<T> read() {
        ...
        try (var streamReader = new InputStreamReader(getUrl().openStream(), StandardCharsets.UTF_8);
            ...
            return Match(parseReader(jsonReader)).of(
                Case($(JsonElement::isJsonArray), j -> StreamEx.of(gson.fromJson(j, ReflectionUtils.castToArray(entityClass)))),
                Case($(), j -> StreamEx.of(gson.fromJson(j, ReflectionUtils.castToObject(entityClass))))
            );
