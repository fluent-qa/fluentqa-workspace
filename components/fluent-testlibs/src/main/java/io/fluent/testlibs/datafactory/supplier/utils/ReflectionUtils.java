...
import io.fluent.testlibs.datafactory.supplier.data.FieldName;
import io.fluent.testlibs.datafactory.supplier.data.Source;
import io.fluent.testlibs.datafactory.supplier.model.TypeMappings;
...
public class ReflectionUtils {
  ...
  public static <T> Class<T[]> castToArray(final Class<T> entityClass) {
    return (Class<T[]>)
        (entityClass.isArray() ? entityClass : Array.newInstance(entityClass, 1).getClass());
  }
  ...
  public static <T> URL getSourcePath(final Class<T> entity) throws IOException {
    return getSourcePath(
        ofNullable(entity.getDeclaredAnnotation(Source.class)).map(Source::path).orElse(""));
  }
  ...
  public static URL getSourcePath(final String path) throws IOException {
    return ofNullable(Try.of(() -> new URL(path)).getOrElseGet(ex -> getSystemResource(path)))
        .orElseThrow(
            () -> new IOException("Unable to access resource specified by " + path + " path"));
  }

  public static <T> StreamEx<T> streamOf(final T data) {
    if (isNull(data)) {
      throw new IllegalArgumentException(
          "Nothing to return from data supplier. Test will be skipped.");
    }
  ...
  @SuppressWarnings("unchecked")
  private static Tuple2<Class<?>, String> getTestAnnotationMetaData(
  ...
  @SuppressWarnings("unchecked")
  private static <T> Class<T> findParentDataSupplierClass(
      final Method testMethod, final Class<T> testClass) {
    return (Class<T>)
        ofNullable(testMethod)
  ...
  private static Function<Tuple2<Method, Reflections>, Class<?>> findParentDataSupplierClass() {
    return t ->
        StreamEx.of(t._2.getSubTypesOf(t._1.getDeclaringClass()))
  ...
  private static Predicate<Tuple2<Method, DataSupplier>> hasDataSupplierMethod(
      final String targetMethodName) {
    return metaData ->
        nonNull(metaData._2)
            && (metaData._2.name().equals(targetMethodName)
                || metaData._1.getName().equals(targetMethodName));
  }
}
