...
import io.fluent.testlibs.datafactory.supplier.converters.*;
import io.fluent.testlibs.datafactory.supplier.converters.IConverter;
import java.util.*;
...
public class XlsxReader<T> implements DataReader<T> {
  ...
  public StreamEx<T> read() {
    try (var workbook = WorkbookFactory.create(getUrl().openStream())) {
      ...
      var headers =
          StreamEx.of(sheet.getRow(startIndex).cellIterator())
              .toMap(cell -> formatter.formatCellValue(cell).trim(), Cell::getColumnIndex);

      var cellMappers =
          StreamEx.of(entityClass.getDeclaredFields())
      ...
      throw new IllegalArgumentException(
          format("Unable to read XLSX data to %s.", entityClass), ex);
    }
  }
  ...
  public T initEntity(final List<XlsxCellMapper<T>> mappers) {
    var hasDefaultConstructor =
        StreamEx.of(entityClass.getDeclaredConstructors())
            .anyMatch(constructor -> constructor.getParameterCount() == 0);
    if (!hasDefaultConstructor) {
      throw new IllegalStateException(
          format("%s must have default constructor.", entityClass.getSimpleName()));
    }
