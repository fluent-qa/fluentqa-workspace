...
import io.fluent.testlibs.datafactory.supplier.utils.ServiceLoaderUtils;
...
public class DataSupplierAspect {
  ...
  private static final List<DataSupplierInterceptor> DATA_SUPPLIERS =
      ServiceLoaderUtils.load(
          DataSupplierInterceptor.class, DataSupplierAspect.class.getClassLoader());
  private static final List<IAnnotationTransformerInterceptor> ANNOTATION_TRANSFORMERS =
      ServiceLoaderUtils.load(
          IAnnotationTransformerInterceptor.class, DataSupplierAspect.class.getClassLoader());

  @Before(
      "execution(@org.testng.annotations.DataProvider * io.github.sskorol.core.DataProviderTransformer.*(..))")
  public void beforeDataProviderCall(final JoinPoint joinPoint) {
    DATA_SUPPLIERS.forEach(
        ds ->
            ds.beforeDataPreparation(
                (ITestContext) joinPoint.getArgs()[0], (ITestNGMethod) joinPoint.getArgs()[1]));
  }
  ...
  public void afterDataProviderCall(final JoinPoint joinPoint) {
    DATA_SUPPLIERS.forEach(
        ds ->
            ds.afterDataPreparation(
                (ITestContext) joinPoint.getArgs()[0], (ITestNGMethod) joinPoint.getArgs()[1]));
  }

  @Around("execution(* io.github.sskorol.core.DataProviderTransformer.getMetaData(..))")
  public DataSupplierMetaData onDataPreparation(final ProceedingJoinPoint joinPoint)
      throws Throwable {
    final DataSupplierMetaData metaData =
        (DataSupplierMetaData) joinPoint.proceed(joinPoint.getArgs());
  ...
  public void beforeTransformationCall(final JoinPoint joinPoint) {
    ...
    Match(args[0])
        .of(
            Case(
                $(ITestAnnotation.class::isInstance),
                arg ->
                    run(
                        () ->
                            callTransformer(
                                at ->
                                    at.transform(
                                        ...
                                        (Method) args[3])))),
            Case(
                $(IFactoryAnnotation.class::isInstance),
                arg ->
                    run(
                        () ->
                            callTransformer(
                                at -> at.transform((IFactoryAnnotation) arg, (Method) args[1])))),
            Case(
                $(IConfigurationAnnotation.class::isInstance),
                arg ->
                    run(
                        () ->
                            callTransformer(
                                at ->
                                    at.transform(
                                        ...
                                        (Method) args[3])))),
            Case(
                $(IDataProviderAnnotation.class::isInstance),
                arg ->
                    run(
                        () ->
                            callTransformer(
                                at ->
                                    at.transform(
                                        (IDataProviderAnnotation) arg, (Method) args[1])))),
            Case(
                $(IListenersAnnotation.class::isInstance),
                arg ->
                    run(
                        () ->
                            callTransformer(
                                at -> at.transform((IListenersAnnotation) arg, (Class) args[1])))));
  }
