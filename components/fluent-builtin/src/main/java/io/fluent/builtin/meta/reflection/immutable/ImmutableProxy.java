...
import io.fluent.builtin.meta.reflection.immutable.collection.DeepImmutableCollection;
import io.fluent.builtin.meta.reflection.immutable.collection.DeepImmutableList;
import io.fluent.builtin.meta.reflection.immutable.collection.DeepImmutableMap;
import io.fluent.builtin.meta.reflection.immutable.collection.DeepImmutableSet;
import io.fluent.builtin.meta.reflection.ClassUtils;
import io.fluent.builtin.meta.reflection.ClassValues;
import io.fluent.builtin.meta.reflection.PropertyUtils;
import io.fluent.builtin.meta.reflection.RecordUtils;
...
public final class ImmutableProxy {
  ...
  private static final ClassValue<Class<?>> immutableProxyClassCache =
      ClassValues.create(ImmutableProxy::createProxyClass);
    ...
    if (isImmutable(instance)) {
      ...
      if (isOptionEnabled(options, ImmutableProxyOption.ALLOW_CLONING_RECORDS)) {
        ...
        throw new IllegalArgumentException(
            instance.getClass()
                + " is a record that potentially contains mutable components."
                + " Consider using ImmutableProxy.create(bean, "
                + ImmutableProxyOption.class.getSimpleName()
                + "."
                + ImmutableProxyOption.ALLOW_CLONING_RECORDS
  ...
  private static <T> void assertPublicMethodsAreNotFinal(Class<T> clazz) {
    for (Method method : clazz.getMethods()) {
      if (Modifier.isFinal(method.getModifiers())) {
        ...
        throw new IllegalArgumentException(
            "Cannot create an immutable proxy for "
                + clazz
                + ". "
                + "Method "
                + method
  ...
  private static ElementMatcher<MethodDescription> isAnnotatedWith(
  ...
  private static boolean isAnnotatedWith(
      SignatureToken methodSignature, TypeDefinition type, Class<? extends Annotation> annotation) {
  ...
  private static boolean hasMethodAnnotatedWith(
      SignatureToken methodSignature, TypeDefinition type, Class<? extends Annotation> annotation) {
    return !type.getDeclaredMethods()
        .filter(
            hasMethodName(methodSignature.getName())
  ...
  private static boolean isOptionEnabled(
      ImmutableProxyOption[] options, ImmutableProxyOption optionToTest) {
    if (options == null) {
      return false;
    }
