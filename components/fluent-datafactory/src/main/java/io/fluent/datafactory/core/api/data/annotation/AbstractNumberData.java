...
/**
 * 数字相关处理 1. short byte int long 2. double float 3. bigInteger bigDecimal 4. atomicLong
 * atomicInteger
 *
 * @author binbin.hou
 * @date 2019/3/9
...
public abstract class AbstractNumberData {
   ...
   * @return 构建的结果
   */
  private String buildIntegerString(IDataFactoryContext context) {
    // 2. 参数范围的处理
    int minResult = 0;
    int maxResult = 100;
  ...
  private void paramCheck(final DataFactory dataFactory) {
    ...
    if (min > max) {
      throw new DataFactoryRuntimeException(
          "DataFactory.min() is not allow great than dataFactory.max().");
    }
    ...
    if (max > maxAllow) {
      throw new DataFactoryRuntimeException(
          "DataFactory.max() is not allow great than " + maxAllow);
    }
  }
   ...
   * @return 结果
   */
  private String buildDecimalString(IDataFactoryContext context) {
   ...
   * @return 结果
   */
  protected String contactIntDecimal(final IDataFactoryContext context) {
   ...
   * @return 结果
   */
  protected String contactInt(final IDataFactoryContext context) {
