...
class TestTable {
  ...
  static TestTable buildUniqueTable(
      DataSource dataSource, String baseTableName, String colDescsAndConstraints) {
    String tableName = buildUniqueTableName(baseTableName);
    String creationScript = "create table " + tableName + "(" + colDescsAndConstraints + ")";
  ...
  static class TestTableAssert extends AbstractAssert<TestTableAssert, TestTable> {
    ...
    TableAssert withScript(String sqlScript) {
      String description =
          LINE_SEPARATOR + "SQL script: " + LINE_SEPARATOR + sqlScript + LINE_SEPARATOR;
