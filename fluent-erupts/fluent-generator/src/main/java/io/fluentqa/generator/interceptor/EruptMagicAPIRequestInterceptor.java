...
public class EruptMagicAPIRequestInterceptor
    implements RequestInterceptor, AuthorizationInterceptor {
      ...
      if (!isLogin) {
        ...
        if (!metaUserInfo.isSuperAdmin()) {
          ...
          if (StringUtils.isNotBlank(permission)
              && eruptUserService.getEruptMenuByValue(permission) == null) {
            return new JsonBean<Void>(403, NO_PERMISSION);
          }
          // 角色判断
          if (StringUtils.isNotBlank(role)
              && metaUserInfo.getRoles().stream().noneMatch(role::equals)) {
            return new JsonBean<Void>(403, NO_PERMISSION);
          }
        }
      }
  ...
  public boolean allowVisit(
      MagicUser magicUser, MagicHttpServletRequest request, Authorization authorization) {
