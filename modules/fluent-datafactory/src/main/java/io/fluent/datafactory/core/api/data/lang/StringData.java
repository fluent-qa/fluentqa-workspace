package io.fluent.datafactory.core.api.data.lang;
...
public class StringData implements IData<String> {
  ...
  @Override
  public String build(IDataFactoryContext context, Class<String> booleanClass) {
   ...
   * @return 长度
   */
  private int getRandomLength(final IDataFactoryContext context, final ThreadLocalRandom random) {
    int minLenResult = 1;
    int maxLenResult = 10;
  ...
  private void paramCheck(DataFactory dataFactory) {
    ...
    if (minLen > maxLen) {
      throw new DataFactoryRuntimeException(
          "DataFactory.maxLen() not allow less than DataFactory.minLen().");
    }
  }
}
