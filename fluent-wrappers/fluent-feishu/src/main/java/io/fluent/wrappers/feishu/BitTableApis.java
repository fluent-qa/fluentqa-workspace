...
public class BitTableApis {
  ...
  public static void createTableRecord(Client client) throws Exception {
    ...
    fields.put(
        "url",
        Url.newBuilder().link("https://bytedance.feishu.cn/drive/home/").text("我是加多").build());
    //    fields.put("Attachment",
    //        new
    // Attachment[]{Attachment.newBuilder().fileToken("boxcnnIpo5luPZtLWjuyJpuo9yf").build()});

    CreateAppTableRecordReq req =
    ...
    if (resp.getCode() != 0) {
      System.out.println(
          String.format(
              "code:%d,msg:%s,err:%s", resp.getCode(), resp.getMsg(), resp.getRequestId()));
      return;
    }
  ...
  public static void batchCreateTableRecord(Client client) throws Exception {
    ...
    fields.put(
        "tag",
        Url.newBuilder().link("https://bytedance.feishu.cn/drive/home/").text("我是加多").build());
    AppTableRecord record = AppTableRecord.newBuilder().fields(fields).build();

    BatchCreateAppTableRecordReq req =
