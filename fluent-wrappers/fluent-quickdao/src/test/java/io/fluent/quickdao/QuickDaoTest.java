...
import io.fluent.quickdao.service.SqlQueryResponse;
import io.fluent.quickdao.service.SqlRequest;
import io.fluent.quickdao.service.SqlService;
import java.util.Map;
import org.junit.jupiter.api.Test;

public class QuickDaoTest {
  DataSourceSetting setting =
      DataSourceSetting.builder()
  ...
  public void testQueryWithBindParams() {
    ...
    var queryResult = dao.query("select  * from hero where name=@name", params);
    for (Entity entity : queryResult) {
      System.out.println(entity.keySet());
      System.out.println(entity.values());
    }
  ...
  public void testUpsert() {
    Entity e = Entity.create("hero").set("name", "test2").set("secret_name", "100").set("age", 100);
  ...
  public void testSaveOrInsert() {
    Entity e =
        Entity.create("hero").set("name", "test4").set("secret_name", "100").set("age", 9000);
  ...
  public void testGetColumnName() {
    DataSourceSetting setting =
        DataSourceSetting.builder()
            .
            //              url("jdbc:sqlite:sample.db")
            url("jdbc:postgresql://127.0.0.1:5432/qa")
            .username("postgres")
            .password("changeit")
            .build();
    QuickDao dao = QuickDao.createDao(setting);
    ...
    tableInfoQuery.addQueryParameterValue("schema", "workspace");

    var results =
        dao.queryForObjects(
            tableInfoQuery.getTableInfoQuery(), tableInfoQuery.getParameters(), TableColumn.class);
